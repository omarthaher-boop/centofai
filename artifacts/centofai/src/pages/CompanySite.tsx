...
import { ArrowRight, Bot, Globe2, Moon, Smartphone, Sparkles, Sun, Workflow } from "lucide-react";
...
const pageCopy = {
  de: {
    ...
    productsTitle: "Produkte von CentofAi.",
    productsLead: "Eigene digitale Produkte, die konkrete Probleme einfach und zuverlässig lösen.",
  ...
  en: {
    ...
    productsTitle: "Products by CentofAi.",
    productsLead: "Our own digital products designed to solve concrete problems simply and reliably.",
...
export default function CompanySite({ kind }: { kind: PageKind }) {
  ...
  return <div className={`company-site ${theme}`}>
    <header className="company-header">
      ...
      <div className="ps-actions">
        ...
        <Link href={`${base}/${lang === "de" ? "projekt-starten" : "start-a-project"}`} className="ps-gradient-btn">{t.nav.start}<ArrowRight size={17}/></Link>
      </div>
    </header>
    ...
    <main>
      <section className="company-hero ps-shell">
        ...
        <div className="company-visual" aria-hidden="true">
          {kind === "services" || kind === "tools" ? <Workflow size={92}/> : kind === "products" ? <Smartphone size={92}/> : <Sparkles size={92}/>} 
      ...
      {kind === "products" && <section className="ps-shell company-section"><Link className="company-product-card" href={`${base}/${lang === "de" ? "produkte/fahrtdoc" : "products/fahrtdoc"}`}><div><small>CENTOF.AI PRODUCT</small><h2>FahrtDoc</h2><p>{lang === "de" ? "Fahrten einfach erfassen, dokumentieren und exportieren." : "Track, document and export trips with less effort."}</p></div><Smartphone size={48}/></Link></section>}

      {["websites","apps","tools"].includes(kind) && <section className="ps-shell company-section"><div className="company-feature-list">{(kind === "websites" ? ["Responsive Design","DE / EN","SEO & Performance","Individuelle Funktionen"] : kind === "apps" ? ["iOS & Android","Push & Login","APIs & Backend","App Store Publishing"] : ["Automatisierungen","KI-Assistenten","Daten & APIs","Workflows & Integrationen"]).map(x => <div key={x}><CheckMark/>{x}</div>)}</div><Link href={`${base}/${lang === "de" ? "projekt-starten" : "start-a-project"}`} className="ps-gradient-btn">{t.nav.start}<ArrowRight size={17}/></Link></section>}
    </main>
